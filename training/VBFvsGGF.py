...
branches_NN = ["pairType","nleps", #general
               "dau1_MVAisoNew", "dau2_MVAisoNew","dau1_iso", "dau2_iso", "dau1_pt", "dau2_pt", #tau candidates
               "bjet1_bID", "bjet2_bID", "bjet1_pt", "bjet2_pt", "bjet1_eta", "bjet2_eta", "dib_dEtaSign", "nbjetscand",# b jet candidates
               "VBFjj_deltaEta", "VBFjj_mass", "VBFjet1_pt", "VBFjet2_pt", "VBFjet1_eta", "VBFjet2_eta", "VBFjet1_btag", "VBFjet2_btag", "VBFjj_HT","VBFjj_dEtaSign",# VBF candidates
               "njets", "njets20", "jj_deltaEta", "jet5_VBF_eta", "jet5_VBF_pt","jet5_VBF_btag","jet3_pt", "jet3_eta","jet4_pt", "jet4_eta", "jj_mass" #additional jets
               "tauH_mass", "tauH_pt", "tauH_SVfit_mass", "tauH_SVfit_pt" #Htt
               "bH_mass", "bH_pt","bH_VBF1_deltaEta", #Hbb
               "isBoosted","pass_trigger", "isVBF", "isVBFtrigger" #flags
               "dau1_z","dau2_z","bjet1_z", "bjet2_z", "tauH_z", "bH_z", "HH_z", "jet5_VBF_z", "jet3_z", "jet4_z" #centrality
]
...
parser.add_argument('--channel', dest='channel', help='which channel', default=2)
parser.add_argument('--selection', dest='selection', help='type-trigger-baseline-looseVBF-tightVBF',default="")
...
parser.add_argument('--cv_BDT', dest = 'cv_BDT', help='use cross validation to estimate the performance', action='store_true', default=False)
parser.add_argument('--optimize', dest = 'optimize', help='optimize neurons', action='store_true', default=False)
parser.add_argument('--optimize_BDT', dest = 'optimize_BDT', help='optimize BDT', action='store_true', default=False)

parser.add_argument('--epochs', dest = 'epochs', help='choose the best model with early stopping', action='store_true', default=False)

parser.add_argument('--show', dest = 'show', help='show plot', action='store_true', default=False)
...
sigTree= load_data.load_chain(sigDir, "HTauTauTree", branches_NN,args.channel,selection)
bkgTree = load_data.load_chain(bkgDir, "HTauTauTree", branches_NN, args.channel,selection)
...
if args.optimize:
    ...
    methods.opt_neurons(input, output, neurons, branches_size)

elif args.cv:
    neurons = [100,50,10]
    methods.cv_NN(input, output, neurons, branches_size)
    ...
    methods.cv_BDT(input, output, params,args.show, branches_cleaned)

elif args.epochs:
    neurons=[100, 10]
    methods.best_early_stopping(input, output, args, neurons, branches_size, args.show)
